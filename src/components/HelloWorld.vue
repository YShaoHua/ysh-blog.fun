<template>
  <div class="todo-box">
    ...
    <div class="todo-middle todo-general" v-for="(item,index) in informationList" :key="index">
      <input type="checkbox" class="check-box" v-model="informationList[index].check">
      <input class="todo-input" v-model="informationList[index].text">
      <p class="delete-todo" @click="deleteTodo(index)">X</p>
    </div>
    <div class="todo-bottom" v-show="informationList.length > 0">
      ...
      <ul>
        <li
          ...
          :class="className === item ? 'add-border' : ''"
        >
          <a>{{item}}</a>
        </li>
      </ul>
...
<style lang="stylus">
.add-border {
  border: 1px solid rgba(175, 47, 47, 0.3) !important;
}
